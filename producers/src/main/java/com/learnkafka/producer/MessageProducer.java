...
public class MessageProducer {
    ...
    public static void main(String[] args) throws InterruptedException, ExecutionException, TimeoutException {
        String example = "Hola Hola Hola Como Como Como Hola a e r f n i w a";

        // 1. Procesamos el String usando Streams
        List<Map.Entry<String, Long>> sortedFrequencies = Arrays.stream(example.split("\\s+"))
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting())) // Agrupar y contar
                .entrySet()
                .stream()
                .sorted(Map.Entry.<String, Long>comparingByValue().reversed()) // Ordenar de mayor a menor
                .collect(Collectors.toList());

        // 2. Extraer resultados
        if (sortedFrequencies.size() >= 2) {
            System.out.println("Palabra más repetida: " + sortedFrequencies.get(0).getKey() +
                    " (" + sortedFrequencies.get(0).getValue() + " veces)");

            System.out.println("Segunda más repetida: " + sortedFrequencies.get(1).getKey() +
                    " (" + sortedFrequencies.get(1).getValue() + " veces)");
        }
